...
#define FUSB301_ID_GROUND   0
#define FUSB301_ID_FLOAT    1
...
struct fusb301_chip {
	...
	struct delayed_work check_id_state;
...
static int fusb301_enable_device(struct fusb301_chip *chip, bool enable)
{

	if (chip->device_enabled != enable) {
		chip->device_enabled = enable;

		if (enable) {
			if (chip->irq_id) {
				disable_irq_wake(chip->irq_id);
				disable_irq(chip->irq_id);
			}
		}
		else {
			if (chip->irq_id) {
				enable_irq(chip->irq_id);
				enable_irq_wake(chip->irq_id);
...
#ifdef CONFIG_LGE_USB_TYPE_C
		ret = snprintf(buf, PAGE_SIZE, "FACTORY(%d)\n", chip->type);
#else
...
#ifdef CONFIG_LGE_USB_TYPE_C
static int fusb301_power_set_hc(struct power_supply *psy, int state)
{
	const union power_supply_propval ret = {state,};
	if (psy->set_property)
		return psy->set_property(psy, POWER_SUPPLY_PROP_USB_HC, &ret);
	return -ENXIO;
}
#endif
...
static void fusb301_src_detected(struct fusb301_chip *chip)
{
	...
	if(chip->dev_id > FUSB301_REV10)
		fusb301_power_set_hc(chip->usb_psy, USB_CC_SINK_MODE);
#endif
...
#ifdef CONFIG_LGE_USB_TYPE_C
	if(chip->dev_id > FUSB301_REV10)
		fusb301_power_set_hc(chip->usb_psy, USB_CC_DBG_ACC_MODE);
#else
	/*
	 * TODO
	 * need to implement
...
#ifdef CONFIG_LGE_USB_TYPE_C
		if(chip->dev_id > FUSB301_REV10)
			fusb301_power_set_hc(chip->usb_psy, USB_CC_NO_MODE);
#endif
...
#ifdef CONFIG_LGE_USB_TYPE_C
static bool fusb301_pwd_acc_detected(struct fusb301_chip *chip)
{
	struct device *cdev = &chip->client->dev;
	int rc = 0;
	...
	if (chip->pwd_trycnt < FUSB301_MAX_TRY_COUNT) {
		rc = fusb301_set_chip_state(chip,
					FUSB_STATE_ERROR_RECOVERY);
		if (IS_ERR_VALUE(rc)) {
			dev_err(cdev, "%s: failed to set error recovery state\n",
...
static void fusb301_attach(struct fusb301_chip *chip)
{
	...
	case FUSB301_TYPE_PWR_ACC:
#ifdef CONFIG_LGE_USB_TYPE_C
		if (fusb301_pwd_acc_detected(chip))
			break;
#endif
		/*
...
static void fusb301_work_handler(struct work_struct *work)
{
	...
	if (IS_ERR_VALUE(rc)) {
		...
		fusb301_enable_device(chip, false);
		goto work_unlock;
	}
...
static void fusb301_check_id_state(struct work_struct *work)
{
	struct fusb301_chip *chip =
		container_of(work, struct fusb301_chip, check_id_state.work);
	int id;

	mutex_lock(&chip->mlock);

	id = gpio_get_value(chip->pdata->id_gpio);
	pr_info("%s : id=%d \n", __func__, id);

	if (chip->id_state != id) {
		pr_info("%s : change chip->id_state=%d and power_supply_set_usb_otg(%d) \n", __func__, id, !id);
		chip->id_state = id;
		power_supply_set_usb_otg(chip->usb_psy, !id);
	}
	else {
		pr_info("%s : ignore... \n", __func__);
	}
...
static irqreturn_t fusb301_id_interrupt(int irq, void *data)
{
	struct fusb301_chip *chip = (struct fusb301_chip *)data;
	int id;
	...
	id = gpio_get_value(chip->pdata->id_gpio);
	if (chip->device_enabled) {
		pr_err("%s : chip is enabled. So do not handle this irq. \n", __func__);
		return IRQ_HANDLED;
	}

	if (chip->id_state != id) {
		cancel_delayed_work_sync(&chip->check_id_state);
		queue_delayed_work(chip->cc_wq, &chip->check_id_state, msecs_to_jiffies(200));
	}
...
static int fusb301_init_gpio(struct fusb301_chip *chip)
{
	...
	if (gpio_is_valid(chip->pdata->int_gpio)) {
		rc = gpio_request(chip->pdata->int_gpio, "fusb301_int_gpio");
		if (rc)
		...
		if (rc)
			dev_err(cdev, "unable to direction_input int_gpio %d\n",
			chip->pdata->int_gpio);
	}
	else {
	...
	/* DETECT_CC gpio */
	if (gpio_is_valid(chip->pdata->id_gpio)) {
		rc = gpio_request(chip->pdata->id_gpio, "fusb301_id_gpio");
		if (rc)
			dev_err(cdev, "unable to request id_gpio %d\n",
				chip->pdata->id_gpio);

		rc = gpio_direction_input(chip->pdata->id_gpio);

		if (rc)
			dev_err(cdev, "unable to direction_input id_gpio %d\n",
				chip->pdata->id_gpio);
	}
	else {
		dev_err(cdev, "id_gpio %d is not valid\n",
			chip->pdata->id_gpio);
		rc = -EINVAL;
	}
...
static int fusb301_parse_dt(struct fusb301_chip *chip)
{
	...
	data->id_gpio = of_get_named_gpio(dev_node,
                "fusb301,id-gpio", 0);
	if (data->id_gpio < 0) {
		dev_err(cdev, "id_gpio is not available\n");
	}
	...
	dev_info(cdev, "gpio:%d init_mode:%d dfp_power:%d toggle_time:%d\n",
			data->int_gpio, data->init_mode, data->dfp_power, data->dttime);
			...
			const struct i2c_device_id *id)
{
	...
	chip->id_state = FUSB301_ID_FLOAT;
	chip->device_enabled = true;
	chip->type = FUSB301_TYPE_INVALID;
	...
	chip->irq_id = gpio_to_irq(chip->pdata->id_gpio);
	if (chip->irq_id < 0) {
		dev_err(cdev, "could not register id_gpio\n");
	}
	else {
		irq_set_status_flags(chip->irq_id,IRQ_NOAUTOEN);
		ret = devm_request_irq(cdev, chip->irq_id,
					fusb301_id_interrupt,
					IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING,
					"fusb301_id_irq", chip);
		if (ret) {
			dev_err(cdev, "failed to reqeust IRQ\n");
			chip->irq_id = 0;
			chip->pdata->id_gpio = 0;
		}
		INIT_DELAYED_WORK(&chip->check_id_state, fusb301_check_id_state);
	}
...
#ifdef CONFIG_LGE_USB_TYPE_C
	if (usb_psy)
		fusb301_power_set_hc(usb_psy, USB_CC_DBG_ACC_MODE);
#endif
...
static int fusb301_remove(struct i2c_client *client)
{
	...
	if (chip->irq_id > 0)
		devm_free_irq(cdev, chip->irq_id, chip);
