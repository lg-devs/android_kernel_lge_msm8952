...
static const u8 the_lge_report_descriptor[] = {
...
static struct hid_descriptor charge_only_desc = {
	...
	.desc[0].wDescriptorLength  =
			cpu_to_le16(sizeof(the_lge_report_descriptor)),
};
		...
		const struct usb_ctrlrequest *ctrl)
{
	...
	pr_debug("charge_only_setup crtl_request : bRequestType:0x%x",
			ctrl->bRequestType);
	pr_debug("bRequest:0x%x Value:0x%x\n",
			ctrl->bRequest, value);
	...
	case ((USB_DIR_IN | USB_TYPE_STANDARD | USB_RECIP_INTERFACE) << 8
		...
		case HID_DT_REPORT:
			...
			length = min_t(unsigned short, length,
					sizeof(the_lge_report_descriptor));
			memcpy(req->buf, the_lge_report_descriptor, length);
			goto respond;
			break;
...
charge_only_function_bind(struct usb_configuration *c, struct usb_function *f)
{
	...
	/* copy descriptors */
	f->fs_descriptors = usb_copy_descriptors(charge_only_fs_descriptors);
	if (!f->fs_descriptors)
		goto fail;

	if (gadget_is_dualspeed(c->cdev->gadget)) {
		f->hs_descriptors =
			usb_copy_descriptors(charge_only_hs_descriptors);
		if (!f->hs_descriptors)
			goto fail;
	}
