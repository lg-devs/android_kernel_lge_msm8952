...
int send_usb_storage_limited(void) {
	...
	if ((soc >= 0) && (soc <= 15)) {
		...
		pr_info("%s: (%d)storage limited uevent sent\n", __func__, soc);
		return 1;
	} else {
		pr_info("%s: (%d)at least a valid battery status ...\n", __func__, soc);
		return 0;
	}
}
#endif
...
#if defined(CONFIG_LGE_USB_DIAG_LOCK) && defined(CONFIG_LGE_USB_G_ANDROID)
