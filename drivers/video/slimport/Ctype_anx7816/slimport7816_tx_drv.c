...
#define AUDIO_MUTE_BAN_DELAY 800
bool audio_restart_flag;
#endif
...
#define hdmi_rx_set_hpd(enable) \
	pr_info("%s %s : %d <= <%pS> \n", LOG_TAG, __func__, enable, __builtin_return_address(0)); \
...
{
	if (!enable && !slimport_is_check()) {
		pr_info("%s : slimport doesn't connect",__func__);
		return;
	}
...
void sp_tx_initialization(void)
{
	...
	sp_write_reg_or(TX_P0, AUX_CTRL2, 0x08); //add by jiaocx@20140917, enable aux double diff output
	...
	*/
	if(is_anx_dongle()){
		...
		if ((data_buf[0] == 0x37) && (data_buf[1] == 0x37)
			...
			sp_tx_aux_dpcdread_bytes(0x00, 0x05, 0x21, 1, &c);
	}

	sp_tx_aux_dpcdread_bytes(0x00, 0x00, DPCD_MAX_LINK_RATE, 1, &c1);

	if (c <= c1)
		c = c1;
	*bw = c;
	pr_info("%s %s : RX max link bw: %x\n", LOG_TAG, __func__,
		(unsigned int)*bw);
...
static void sp_tx_lvttl_bit_mapping(void)
{
	...
	if (!down_sample_en) {
		...
		/* only YCbCr4:2:2 is used for 4K video output */
		if ((colorspace == 0x20) || (colorspace == 0x40) ||
				(colorspace == 0x00)) {
...
int slimport_link_training(void)
{
	unchar temp_value, return_value, c;

	if (!slimport_is_check()) {
		pr_err("%s %s : slimport is power down", LOG_TAG, __func__);
		return 1;
	}
	...
	switch(sp_tx_LT_state) {
		...
		case LT_START:
			if(sp_tx_test_lt) {
				...
				if (LINK_6P75G == g_changed_bandwidth)
					sp_write_reg(TX_P0, SP_TX_LT_SET_REG, 0x03);
			}
...
int slimport_config_video_output(void)
{
	unchar temp_value;
	...
	case VO_WAIT_TX_VIDEO_STABLE:
		...
		case VO_FINISH:
			...
			//sp_tx_config_packets(AVI_PACKETS);
			//sp_tx_enable_video_input(1);
			if (sp_rx_bandwidth < 0x14) {
				pr_info("%s: get delay 3 sec to sync with low bandwidth dongle", __func__);
				msleep(3000);
			}
...
void slimport_hdcp_process(void)
{
	...
	case HDCP_FINISH:
		...
		//sp_tx_aux_dpcdwrite_byte(0x00, 0x05, DPCD_SPECIFIC_INTERRUPT1, 0xff);
		break;
	case HDCP_FAILE:
...
void slimport_state_process (void)
{
	...
	case STATE_VIDEO_OUTPUT:
		ret = slimport_config_video_output();
		if (ret) {
			sp_tx_set_sys_state(STATE_WAITTING_CABLE_PLUG);
			break;
		}
...
static void hpd_irq_process(void)
{
	...
	if (!sp_tx_get_dp_connection()) {
		pr_err("Cannot get dp connection\n");
		sp_tx_set_sys_state(STATE_WAITTING_CABLE_PLUG);
		g_need_clean_status = 1;
		return;
	}
	sp_tx_aux_dpcdread_bytes(0x00, 0x02, 0x00, 6, data_buf);
	/* Slimport state only decide by hardware irq */
	/*
	if (!(data_buf[0] & 0xbf)) {
		sp_tx_set_sys_state(STATE_WAITTING_CABLE_PLUG);
		return;
	}*/
	pr_err("+++++++++++++Get HPD IRQ %x\n", (int)data_buf[1]);
...
static void sp_tx_hpd_int_handler(unchar hpd_source)
{
	...
	case HPD_CHANGE:
		...
		if(COMMON_INT4 & HPD_IRQ)
			hpd_irq_process();
		sp_read_reg(TX_P0, SP_TX_SYS_CTRL3_REG, &u);
		if(u & HPD_STATUS){
			//IRQ
			if(COMMON_INT4 & HPD_IRQ)
				hpd_irq_process();
			...
			sp_read_reg(TX_P0, SP_TX_SYS_CTRL3_REG, &u);
			if(u & HPD_STATUS){
				pr_info("%s %s : HPD: Unstable => Re-connect\n", LOG_TAG, __func__);
				system_power_ctrl(0);
				/* hdmi_rx_set_hpd(0); */
				/* sp_tx_set_sys_state(STATE_WAITTING_CABLE_PLUG); */
			}
