...
int    broadcast_fc8300_drv_if_set_channel(struct broadcast_dmb_set_ch_info *udata)
{
    ...
    if(udata->mode != 2) {
        ...
        ret = tunerbb_drv_fc8300_tuner_select(ISDBT_1SEG);
        print_log(NULL, "[YK][fc8300] tuner select 1seg %d\n",udata->mode);
    } else {
        currentBroadCast = UHF_13SEG;
        ret = tunerbb_drv_fc8300_tuner_select(ISDBT_13SEG);
        print_log(NULL, "[YK1][fc8300] tuner select 13seg %d\n", udata->mode);
    }
#else
    ...
    if(udata->segment == 13) {
        //currentBroadCast = UHF_13SEG;
        ret = tunerbb_drv_fc8300_tuner_select(ISDBT_13SEG);
        print_log(NULL, "[fc8300] tuner select 13seg\n");
    } else {
        //currentBroadCast = UHF_1SEG;
        ret = tunerbb_drv_fc8300_tuner_select(ISDBT_1SEG);
        print_log(NULL, "[fc8300] tuner select 1seg\n");
    }
