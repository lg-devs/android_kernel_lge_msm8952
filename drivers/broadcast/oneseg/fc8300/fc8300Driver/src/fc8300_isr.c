...
static void fc8300_data(HANDLE handle, DEVICEID devid, u8 buf_int_status)
{
	...
	if (buf_int_status == 1) {
        ...
        print_log(NULL, "[YK] fc8300_data 0x%x\n", buf_int_status);
}
...
void fc8300_isr(HANDLE handle)
{
    ...
    if (buf_int_status) {
        ...
        // Modified by harold.kim 20160701 for SPI debugging
        print_log(NULL, "[YK] fc8300_isr yk_read_cnt %d\n", yk_read_cnt);
        ...
        if (buf_int_status) {
            // Modified by harold.kim 20160701 for SPI debugging
            print_log(NULL, "[YK] buf_int_status1 %d %d\n", buf_int_status, yk_read_cnt);
        ...
        {
            //print_log(NULL, "[YK] overrun_check_count %d\n", overrun_check_count);
        ...
        print_log(NULL, "[YK] buf_int_status!! 0x%x\n", buf_int_status);
#endif // BBM_I2C_TSIF
