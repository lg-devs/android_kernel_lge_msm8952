...
int tunerbb_drv_fc8300_init(int mode)
{
    ...
    print_log(0, "[YK2] FC8300 tunerbb_drv_fc8300_init\n");
#ifndef BBM_I2C_TSIF
...
int tunerbb_drv_fc8300_tuner_select(int mode)
{
        s32 res = BBM_OK;
        if (gTunermode != mode) {
            res = bbm_com_tuner_select(NULL, DIV_BROADCAST, FC8300_TUNER, mode);
            print_log(NULL, "FC8300 tunerbb_drv_fc8300_tuner_select mode : %d\n", mode);
        }
...
int tunerbb_drv_fc8300_Get_SignalInfo(struct fc8300Status_t *st, s32 brd_type)
{
    ...
    st->cn= (cn <= 127)?cn * 25:0;
    //print_log(NULL, "[FC8300] CN : %d, st->cn : %d\n", cn, st->cn);
    //st->agc = agc;
    ...
    if (gTunermode == 0)
    {
        print_log(NULL, "[FC8300] B/C Layerinfo(0xffff) in oneseg only mode\n");
        st->layerinfo_b = 0xffff;
        st->layerinfo_c = 0xffff;
    }
    ...
    //print_log(NULL, "[FC8300] numOfSegA(%d), numOfSegB(%d), numOfSegC(%d)\n", num_of_seg_layer_a, num_of_seg_layer_b, num_of_seg_layer_c);
	
    if((num_of_seg_layer_a >= 12) && (num_of_seg_layer_a != 0x0f) ) {
	 //print_log(NULL, "[FC8300] Layer A is selected as FULLSEG Layer\n");
	 ...
	 //print_log(NULL, "[FC8300] Layer C is selected as FULLSEG Layer\n");
	 ...
	 //print_log(NULL, "[FC8300] Layer B is selected as FULLSEG Layer\n");
	 ...
	 //print_log(NULL, "[FC8300] any layer isn't selected as FULLSEG Layer\n");
	 st->ber_fullseg = st->ber_b;
        st->per_fullseg = st->per_b;
        st->err_tsp_fullseg = dm.vit_b_ber_err_rsps;
        st->total_tsp_fullseg = dm.vit_b_ber_rxd_rsps;
    ...
    //print_log(NULL, "[FC8300]ant_oneseg : %d,  ber_oneseg : %d, ant_fullseg : %d,  ber_fullseg : %d, switching : %d, st->cn=%d\n", st->antenna_level_oneseg, st->ber_1seg, st->antenna_level_fullseg, st->ber_fullseg, st->fullseg_oneseg_flag, st->cn);
