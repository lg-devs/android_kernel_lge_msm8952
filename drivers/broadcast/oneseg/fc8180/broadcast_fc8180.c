...
#ifdef FEATURE_DMB_USE_EXTLNA
        gpio_set_value(IsdbCtrlInfo.ext_lna0_gpio, 0); /* GS */
        gpio_set_value(IsdbCtrlInfo.ext_lna1_gpio, 1); /* EN */
#endif
...
#ifdef FEATURE_DMB_USE_EXTLNA
        gpio_set_value(IsdbCtrlInfo.ext_lna0_gpio, 0); /* GS */
        gpio_set_value(IsdbCtrlInfo.ext_lna1_gpio, 0); /* EN */
#endif
...
static int broadcast_Isdb_config_gpios(void)
{
...
#ifdef FEATURE_DMB_USE_EXTLNA
    IsdbCtrlInfo.ext_lna0_gpio = of_get_named_gpio(IsdbCtrlInfo.pdev->dev.of_node, "fci_spi_fc8180,ext_lna0_gpio",0); /* GS */
    IsdbCtrlInfo.ext_lna1_gpio = of_get_named_gpio(IsdbCtrlInfo.pdev->dev.of_node, "fci_spi_fc8180,ext_lna1_gpio",0); /* EN */
...
#ifdef FEATURE_DMB_USE_EXTLNA
    rc =  gpio_request(IsdbCtrlInfo.ext_lna0_gpio, "1SEG_LNA_GS");
    if(rc < 0) {
        err_count++;
    ...
    rc =  gpio_request(IsdbCtrlInfo.ext_lna1_gpio, "1SEG_LNA_EN");
    if(rc < 0) {
        err_count++;
...
#ifdef FEATURE_DMB_USE_EXTLNA
    gpio_direction_output(IsdbCtrlInfo.ext_lna0_gpio, 0); /* GS */
    gpio_direction_output(IsdbCtrlInfo.ext_lna1_gpio, 0); /* EN */
#endif
