...
wpt_status WDTS_Close(void *pContext)
{
  ...
  WDI_DS_BdMemPoolType *mgmt_memPool;
  WDI_DS_BdMemPoolType *data_memPool;
  tPalDmaMemInfo *mMemInfo;
  tPalDmaMemInfo *dMemInfo;
  uint8_t initial = 0;

  mgmt_memPool = &pClientData->mgmtMemPool;
  data_memPool = &pClientData->mgmtMemPool;

  mMemInfo = (tPalDmaMemInfo *)(((wpt_byte *)mgmt_memPool->pVirtBaseAddress) -
                               sizeof(tPalDmaMemInfo));
  dMemInfo = (tPalDmaMemInfo *)(((wpt_byte *)data_memPool->pVirtBaseAddress) -
                               sizeof(tPalDmaMemInfo));

  printk(" ----- Mgmt ----- \n");
  printk("Length: %d phyAddr: %x \n", mMemInfo->length, mMemInfo->phyAddr);

  printk(" ----- Data ----- \n");
  printk("Length: %d phyAddr: %x \n", dMemInfo->length, dMemInfo->phyAddr);
  if (0 == dMemInfo->length){
      initial  = 1;
      printk("WDTS_Close ! Corruption ! \n");
  }
  ...
  printk("@@@ Length: %d phyAddr: %x \n", dMemInfo->length, dMemInfo->phyAddr);
  if ((0 == dMemInfo->length) && (0 == initial))
       printk("Corruption after management pool destroy \n");
  /*Destroy the mem pool for mgmt BD headers*/
