                                                                  ...
                                                                  wpt_uint8 numChunks)
{
  ...
  if (WDI_DS_HI_PRI_RES_NUM == numChunks) {
      paddr_m = memPool->pVirtBaseAddress;
  } else if (WDI_DS_LO_PRI_RES_NUM == numChunks) {
      paddr_d = memPool->pVirtBaseAddress;
  }
...
{
  tPalDmaMemInfo *pMemInfo = (tPalDmaMemInfo *)(((wpt_byte *)memPool->pVirtBaseAddress) -
                             sizeof(tPalDmaMemInfo));
  int i;

if (pMemInfo->length == 0) {
    printk("%s: Length of Memory pool %p is 0!\n", __func__,memPool->pVirtBaseAddress);
    printk("paddr_m = %p, paddr_d = %p\n", paddr_m, paddr_d);

    printk("--------------(paddr_m-tPalDmaMemInfo)-------------\n");
    if (paddr_m) {
        for (i = 0; i < 256; i++) {
             printk("%02x ", (wpt_uint8)*((wpt_uint8 *)paddr_m + i - sizeof(tPalDmaMemInfo)));
             if ((i + 1)%16 == 0)
                 printk("\n");
        }
    }

    printk("--------------(paddr_d-tPalDmaMemInfo)-------------\n");
    if (paddr_d) {
        for (i = 0; i < 256; i++) {
            printk("%02x ", (wpt_uint8)*((wpt_uint8 *)paddr_d + i - sizeof(tPalDmaMemInfo)));
            if ((i + 1)%16 == 0)
                printk("\n");
        }
    }
