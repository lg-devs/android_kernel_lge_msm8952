...
#endif
{
	...
	if (ext_csd_rev >= 7) {
       ...
       if (ext_csd_rev >= 6) {
		...
		if (ext_csd_rev >= 7) {
			...
			if (ext_csd_rev >= 8) {
				seq_printf(s, "[486] Barrier support, BARRIER_SUPPORT: 0x%02x\n", ext_csd[486]);
				/* B50: reserved [485:309] */
	...
	seq_printf(s, "[196] Device type, device_type: 0x%02x\n", ext_csd[196]);
	   ...
	   if (ext_csd_rev >= 8) {
	       seq_printf(s, "[184] Strobe Support, STROBE_SUPPORT: 0x%02x\n", ext_csd[184]);
		   seq_printf(s, "[183] Bus width mode, BUS_WIDTH: 0x%02x\n", ext_csd[183]);
	   }
	...
	seq_printf(s, "[177] Boot bus Conditions, boot_bus_conditions: 0x%02x\n", ext_csd[177]);
       ...
       if (ext_csd_rev >= 5) {
		seq_printf(s, "[174] Boot write protection status registers, boot_wp_status: 0x%02x\n", ext_csd[174]);
			...
			if (ext_csd_rev >= 8) {
				seq_printf(s, "[165] Start Sanitize operation , SANITIZE_START: 0x%02x\n", ext_csd[165]);
				seq_printf(s, "[164] Manually start  background operations, BKOPS_START: 0x%02x\n", ext_csd[164]);
			}
	...
	if (ext_csd_rev >= 7)
		...
		if (ext_csd_rev >= 8) {
			seq_printf(s, "[132] Package Case Temperature is controlled, TCASE_SUPPORT: 0x%02x\n", ext_csd[132]);
		}
       if (ext_csd_rev >= 6) {
               int j;
	...
	if (ext_csd_rev >= 8) {
		seq_printf(s, "[32] Flushing of the cache, FLUSH_CACHE: 0x%02x\n", ext_csd[32]);
