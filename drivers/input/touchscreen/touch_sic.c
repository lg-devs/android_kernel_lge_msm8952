...
#include "./touch_sic_test_limits.h"
#include "touch_sic_test_ref.h"

#if USE_ABT_MONITOR_APP
#include <linux/input/touch_sic_abt.h>

u16 frame_num = 0;	// [+ 20150509 ssw] To solve the frame index counting issue
#endif
...
#define fw_i2c_burst_write_size	512
...
static u16 RxSlopeReference[ROW_SIZE];
bool ghost_do_not_reset;

static struct touch_fw_info *info_sic;
static struct touch_firmware_module *fw_sic;
static int is_first_INT = 0;
static int is_second_INT = 0;
	...
	if (ic_status == 0) {	/* NEAR, no need to LPWG Set */
		...
		if (atomic_read(&ts->lpwg_ctrl.is_suspend)
		 == TC_STATUS_SUSPEND) {
			...
			is_second_INT = 1;
			queue_delayed_work(touch_wq, &ts->work_second_int,
					msecs_to_jiffies(50));
			lpwg_status = -1; /* Already Set Doze2 Mode in sic_ts_init */
		}
	} else if (ic_status == 2) {
		sic_ts_power(ts->client, POWER_OFF);
		sic_ts_power(ts->client, POWER_ON);
...
static void ignore_second_int(struct work_struct *work_second_int)
{
	TOUCH_I("%s\n", __func__);
	is_second_INT = 0;
}
...
static ssize_t show_sd(struct i2c_client *client, char *buf)
{
	...
	u16 node_h, node_l;
	u32 wdata, rdata;
	u8  tx_cnt = 18, rx_cnt = 32;
	u32 rx_result, tx_result;
	u32 fail_cnt;
	u32 check_try_cnt = 0;
	u32 max_result = 0, min_result = 99999;

#define MAX_TRY_CNT 50

	TOUCH_I("%s start\n", __func__);

	memset (sicImage, 0, sizeof(sicImage));
	memset (LowerLimit, 0, sizeof(LowerLimit));
	memset (RxSlopeReference, 0, sizeof(RxSlopeReference));

	/* file create , time log */
	write_file(NULL, "", 1, 1);

	/* ReCalibration before Test */
	wdata = 0x20;
	do {
		TOUCH_I("%s ReCalibration before Test\n", __func__);
		usleep(10000);
		DO_SAFE(sic_i2c_write(ts->client, tc_device_ctl,
					(u8 *)&wdata, sizeof(u32)), error);

		DO_SAFE(sic_i2c_read(ts->client,
			tc_status, (u8 *)&rdata, sizeof(u32)), error);
	} while ( rdata == 7 );
	TOUCH_I("%s ReCalibration Complete\n", __func__);

	/* Stop Normal Mode */
	wdata = 0x02;
	DO_SAFE(sic_i2c_write(client, tc_driving_ctl, (u8 *)&wdata, sizeof(u32)), error);

	/* 50ms waiting for normal stop */
	usleep(50000);

	/* touch interrupt disable */
	wdata = 0x00;
	DO_SAFE(sic_i2c_write(client, tc_interrupt_ctl, (u8 *)&wdata, sizeof(u32)), error);

	temp_buf = kzalloc(10240, GFP_KERNEL);
	if (!temp_buf) {
	...
	if (atomic_read(&ts->lpwg_ctrl.is_suspend)) {
		len = sprintf(temp_buf + len, "state=[suspend]. we cannot use I2C, now. Test Result: Fail\n\n");
		write_file(NULL, temp_buf, 0, 1);
		TOUCH_I("%s : %s", __func__, temp_buf);
		return ret;
	}

	len = sprintf(temp_buf, "Test Num                ");
	for (i = 0; i < rx_cnt; i++)
		len += sprintf(temp_buf + len, "%7dr", i+1);
	for (i = 0; i < tx_cnt; i++)
		len += sprintf(temp_buf + len, "%7dt", i+1);
	len += sprintf(temp_buf + len, " Result\n");

	/* TRx GND Short Test */
	wdata = TSP_TEST_TX_RX_GND_SHORT;
	DO_SAFE(sic_i2c_write(ts->client,
		tc_tsp_test_mode_ctl, (u8 *)&wdata, sizeof(u32)), error);

	do {
		usleep(10000);
		DO_SAFE(sic_i2c_read(ts->client,
			tc_tsp_test_mode_sts, (u8 *)&rdata, sizeof(u32)), error);
		check_try_cnt++;
		if (check_try_cnt > MAX_TRY_CNT)
			goto error;
	} while (rdata != 0);

	DO_SAFE(sic_i2c_read(ts->client,
		tc_tsp_test_mode_rx_result, (u8 *)&rx_result, sizeof(u32)), error);

	DO_SAFE(sic_i2c_read(ts->client,
		tc_tsp_test_mode_tx_result, (u8 *)&tx_result, sizeof(u32)), error);

	len += sprintf(temp_buf + len, "[T%d]TRx_GND_Short_Test  ", wdata);

	for (i = 0; i < rx_cnt; i++)
		len += sprintf(temp_buf + len, "%8d", ((rx_result >> i) & 1) ? 0 : 1);
	for (i = 0; i < tx_cnt; i++)
		len += sprintf(temp_buf + len, "%8d", ((tx_result >> i) & 1) ? 0 : 1);
	len += sprintf(temp_buf + len, "%5d\n", (rx_result || tx_result) ? 0 : 1);
	TOUCH_I("%s TRx GND Short Test result (rx_result:%x, tx_result:%x)\n", __func__, rx_result, tx_result);

	trx_to_trx |= rx_result;
	trx_to_trx |= tx_result;

	/* TRx ADV Short Test*/
	wdata = TSP_TEST_TX_RX_ADJ_SHORT;
	DO_SAFE(sic_i2c_write(ts->client,
		tc_tsp_test_mode_ctl, (u8 *)&wdata, sizeof(u32)), error);
	...
	len += sprintf(temp_buf + len, "[T%d]TRx_ADV_Short_Test  ", wdata);
	...
	for (i = 0; i < tx_cnt; i++)
		len += sprintf(temp_buf + len, "%8d", ((tx_result >> i) & 1) ? 0 : 1);
	len += sprintf(temp_buf + len, "%5d\n", (rx_result || tx_result) ? 0 : 1);
	TOUCH_I("%s TRx ADV Short Test result (rx_result:%x, tx_result:%x)\n", __func__, rx_result, tx_result);

	trx_to_trx |= rx_result;
	trx_to_trx |= tx_result;

	/* TRx Open Test*/
	wdata = TSP_TEST_TX_RX_OPEN;
	DO_SAFE(sic_i2c_write(ts->client,
		tc_tsp_test_mode_ctl, (u8 *)&wdata, sizeof(u32)), error);
	...
	len += sprintf(temp_buf + len, "[T%d]TRx_Open_Test       ", wdata);
	...
	for (i = 0; i < tx_cnt; i++)
		len += sprintf(temp_buf + len, "%8d", ((tx_result >> i) & 1) ? 0 : 1);
	len += sprintf(temp_buf + len, "%5d\n", (rx_result || tx_result) ? 0 : 1);
	TOUCH_I("%s TRx Open Test result (rx_result:%x, tx_result:%x)\n", __func__, rx_result, tx_result);

	high_resistance |= rx_result;
	high_resistance |= tx_result;

	write_file (NULL, temp_buf, 0, 1);

	/* Cm measure Test*/
#if 0
	wdata = TSP_TEST_CM_MEASURE;
	DO_SAFE(sic_i2c_write(ts->client,
		tc_tsp_test_mode_ctl, (u8 *)&wdata, sizeof(u32)), error);
	...
	wdata = 0; /* node idx to 0 */
	DO_SAFE(sic_i2c_write(ts->client,
		tc_tsp_test_mode_node_result, (u8 *)&wdata, sizeof(u32)), error);

	j = 0;
	fail_cnt = 0;
	for (i = 0; i < (tx_cnt*rx_cnt)/2+1; i++) {
		DO_SAFE(sic_i2c_read(ts->client,
			tc_tsp_test_mode_node_result, (u8 *)&node, sizeof(u32)), error);
		node_l = (node[0] >> 2);
		node_h = (node[1] >> 2);
		if (j < (tx_cnt * rx_cnt)) {
			len += sprintf(temp_buf + len, "%5d.o", node_l);
			j++;
			if (j != 0 && (j % rx_cnt == 0))
				len += sprintf(temp_buf + len, "\n");
			len += sprintf(temp_buf + len, "%5d.o", node_h);
			j++;
			if (j != 0 && (j % rx_cnt == 0))
				len += sprintf(temp_buf + len, "\n");
		}
	...
	len += sprintf(temp_buf + len, "%5d%5d%5d%5d\n",
			TSP_TEST_CM_MEASURE, 0, fail_cnt, fail_cnt == 0 ? 1:0);
	TOUCH_I("%s CM Test result (%x)\n", __func__, fail_cnt == 0 ? 1:0);
#endif

#if 0
	/* Sensor Speed Test*/
	wdata = TSP_TEST_SENSOR_SPEED;
	DO_SAFE(sic_i2c_write(ts->client,
		tc_tsp_test_mode_ctl, (u8 *)&wdata, sizeof(u32)), error);
	...
	for (i = 0; i < (tx_cnt*rx_cnt)/2+1; i++) {
		DO_SAFE(sic_i2c_read(ts->client,
				tc_tsp_test_mode_node_result, (u8 *)&node, sizeof(u32)), error);
		if (j < (tx_cnt * rx_cnt)) {
			node_l = (node[0] >> 2) + ADC_OFFSET;
			ref_offset = sensor_speed_ref[j]*ADC_VALID_RATIO/100;
			lower_adc = sensor_speed_ref[j] - ref_offset;
			upper_adc = sensor_speed_ref[j] + ref_offset;
			if (node_l < lower_adc
				|| node_l > upper_adc) {
				fail_cnt++;
				len += sprintf(temp_buf + len, "%5d.x", node_l);
			} else {
				len += sprintf(temp_buf + len, "%5d.o", node_l);
			}
			j++;
		...
		if (j < (tx_cnt * rx_cnt)) {
			...
			if (node_h < lower_adc
				|| node_h > upper_adc) {
				fail_cnt++;
				len += sprintf(temp_buf + len, "%5d.x", node_h);
			} else {
				len += sprintf(temp_buf + len, "%5d.o", node_h);
			}
			j++;
	...
	len += sprintf(temp_buf + len, "%5d%5d%5d%5d\n",
			TSP_TEST_SENSOR_SPEED, ADC_VALID_RATIO, fail_cnt, fail_cnt == 0 ? 1:0);
	TOUCH_I("%s Sensor Speed Test result (%x)\n", __func__, fail_cnt == 0 ? 1:0);
	if (fail_cnt > 0)
		full_raw_cap = 0;
	else
		full_raw_cap = 1;
	...
	len += sprintf(temp_buf + len, "%5d%5d%5d%5d\n",
			TSP_TEST_ADC_RANGE, 0, fail_cnt, fail_cnt == 0 ? 1:0);
	TOUCH_I("%s ADC Range Test result (%x)\n", __func__, fail_cnt == 0 ? 1:0);
#endif

	/* Slope Test */
	wdata = TSP_TEST_SLOPE;
	DO_SAFE(sic_i2c_write(ts->client,
		tc_tsp_test_mode_ctl, (u8 *)&wdata, sizeof(u32)), error);
	...
	len = sprintf(temp_buf, "[T%d]Slope_Test          \n",wdata);
	...
	for (i = 0; i < (tx_cnt*rx_cnt)/2+1; i++) {
		...
		node_h = (node[1] >> 2);
		if (j < (tx_cnt * rx_cnt)) {
			sicImage[i/(rx_cnt/2)][j%rx_cnt] = node_l;
			j++;
		}
		if (j < (tx_cnt * rx_cnt)) {
			sicImage[i/(rx_cnt/2)][j%rx_cnt] = node_h;
			j++;
		}
	}

	/* Check Rx channel Average */
	len += sprintf(temp_buf + len, "Rx_average              ");
	fail_cnt = 0;
	for ( j = 0; j< rx_cnt; j++) {
		for ( i = 0; i< tx_cnt; i++) {
			RxSlopeReference[j] += sicImage[i][j];
		}

		RxSlopeReference[j] /= tx_cnt;

		if(RxSlopeReference[j] < (SLOPE_BASE - SLOPE_VALID_OFFSET)
			|| RxSlopeReference[j] > (SLOPE_BASE + SLOPE_VALID_OFFSET)) {
			fail_cnt++;
			len += sprintf(temp_buf + len, "       0");
		} else {
			len += sprintf(temp_buf + len, "       1");
		}
	}

	len += sprintf(temp_buf + len, "\n                        ");
	for ( i = 0; i< rx_cnt; i++) {
		len += sprintf(temp_buf + len, "%8d", RxSlopeReference[i]);
	}

	write_file (NULL, temp_buf, 0, 1);

	/* Check Slope Reference */
	len = sprintf(temp_buf, "\nSlope Start\n");
	for ( j = 0; j< rx_cnt; j++) {
		for ( i = 0; i< tx_cnt; i++) {
			LowerLimit[i][j] = sicImage[i][j] + SLOPE_BASE - RxSlopeReference[j];
		}
	}

	for ( j = 0; j< tx_cnt; j++) {
		len += sprintf(temp_buf + len, "s_%dt                   ", j+1);
		for ( i = 0; i< rx_cnt; i++) {
			ref_offset = sensor_slope_ref[j * rx_cnt + i] * SLOPE_VALID_RATIO / 100;
			lower_adc = sensor_slope_ref[j * rx_cnt + i] - ref_offset;
			upper_adc = sensor_slope_ref[j * rx_cnt + i] + ref_offset;
			len += sprintf(temp_buf + len, "%5d", sicImage[j][i]);
			if (LowerLimit[j][i] < lower_adc || LowerLimit[j][i] > upper_adc) {
				fail_cnt++;
				len += sprintf(temp_buf + len, "(0)");
			} else
				len += sprintf(temp_buf + len, "(1)");
	...
	len += sprintf(temp_buf + len, "Slope Result            %8d%8d%8d\n",
		SLOPE_VALID_RATIO, fail_cnt, fail_cnt == 0 ? 1:0);

	write_file (NULL, temp_buf, 0, 1);

	len = sprintf(temp_buf, "Slope_NEW\n");

	for ( j = 0; j< tx_cnt; j++) {
		len += sprintf(temp_buf + len, "s_%dt_N                 ", j+1);
		for ( i = 0; i< rx_cnt; i++) {
			len += sprintf(temp_buf + len, "%8d", LowerLimit[j][i]);
		}
		len += sprintf(temp_buf + len, "\n");
	}
	...
	len = sprintf(temp_buf, "Slope_%%\n");

	for ( j = 0; j< tx_cnt; j++) {
		len += sprintf(temp_buf + len, "s_%dt%%                  ", j+1);
		for ( i = 0; i< rx_cnt; i++) {
			lower_adc = (int)LowerLimit[j][i] * 1000 / (int)sensor_slope_ref[j * rx_cnt + i];
			len += sprintf(temp_buf + len, "%7d.%d", lower_adc /10, lower_adc % 10);
			if ( lower_adc > max_result )
				max_result = lower_adc;
			else if ( lower_adc < min_result )
				min_result = lower_adc;
		}
		len += sprintf(temp_buf + len, "\n");
	}

	len = sprintf(temp_buf + len, "Slope_%%                 %7d.%d%7d.%d\n",
		max_result / 10, max_result % 10, min_result / 10, min_result % 10);

	TOUCH_I("%s Slope Test result (%x)\n", __func__, fail_cnt == 0 ? 1:0);

	high_resistance |= fail_cnt;

	write_file (NULL, temp_buf, 0, 1);

	/* Raw Capacitance Test */
	wdata = TSP_TEST_RAW_CAPACITANCE;
	DO_SAFE(sic_i2c_write(ts->client,
		tc_tsp_test_mode_ctl, (u8 *)&wdata, sizeof(u32)), error);
	...
	len = sprintf(temp_buf, "[T%d]Raw_Capacitance_Test",wdata);
	...
	for (i = 0; i < (tx_cnt*rx_cnt)/2+1; i++) {
		DO_SAFE(sic_i2c_read(ts->client,
				tc_tsp_test_mode_node_result, (u8 *)&node, sizeof(u32)), error);
		if (j < (tx_cnt * rx_cnt)) {
			node_l = REF_OFFSET - (node[0] >> 2);
			sicImage[i/(rx_cnt/2)][j%rx_cnt] = node_l;
			ref_offset = sensor_raw_cap_ref[j] * RAW_CAP_VALID_RATIO/100;
			lower_adc = sensor_raw_cap_ref[j] - ref_offset;
			upper_adc = sensor_raw_cap_ref[j] + ref_offset;
			if (node_l < lower_adc
				|| node_l > upper_adc) {
				fail_cnt++;
				len += sprintf(temp_buf + len, "%5d(0)", node_l);
			} else {
				len += sprintf(temp_buf + len, "%5d(1)", node_l);
			}
			j++;
			if (j != 0 && (j % rx_cnt == 0) && (j / rx_cnt != 18))
				len += sprintf(temp_buf + len, "\nr_%dt                   ", j/rx_cnt+1);
		}
		if (j < (tx_cnt * rx_cnt)) {
			node_h = REF_OFFSET - (node[1] >> 2);
			sicImage[i/(rx_cnt/2)][j%rx_cnt] = node_h;
			ref_offset = sensor_raw_cap_ref[j] * RAW_CAP_VALID_RATIO/100;
			lower_adc = sensor_raw_cap_ref[j] - ref_offset;
			upper_adc = sensor_raw_cap_ref[j] + ref_offset;
			if (node_h < lower_adc
				|| node_h > upper_adc) {
				fail_cnt++;
				len += sprintf(temp_buf + len, "%5d(0)", node_h);
			} else {
				len += sprintf(temp_buf + len, "%5d(1)", node_h);
			}
			j++;
			if (j != 0 && (j % rx_cnt == 0) && (j / rx_cnt != 18))
				len += sprintf(temp_buf + len, "\nr_%dt                   ", j/rx_cnt+1);
	...
	len += sprintf(temp_buf + len, "\nRaw Cap Result		  %8d%8d%8d\n",
			RAW_CAP_VALID_RATIO, fail_cnt, fail_cnt == 0 ? 1:0);
	...
	len = sprintf(temp_buf, "Raw_Cap_%%\n");

	for ( j = 0; j< tx_cnt; j++) {
		len += sprintf(temp_buf + len, "r_%dt%%                  ", j+1);
		for ( i = 0; i< rx_cnt; i++) {
			lower_adc = (int)sicImage[j][i] * 1000 / (int)sensor_raw_cap_ref[j * rx_cnt + i];
			len += sprintf(temp_buf + len, "%7d.%d",
				lower_adc / 10, lower_adc % 10);
			if ( lower_adc > max_result )
				max_result = lower_adc;
			else if ( lower_adc < min_result )
				min_result = lower_adc;
	...
	len = sprintf(temp_buf+len, "Raw_Cap_%%               %7d.%d%7d.%d\n",
		max_result / 10, max_result % 10, min_result / 10, min_result % 10);

	write_file (NULL, temp_buf, 0, 1);
	write_file (NULL, "", 1, 1);

	TOUCH_I("%s Raw Capacitance Test result (%x)\n", __func__, fail_cnt == 0 ? 1:0);
	if (fail_cnt > 0)
		full_raw_cap = 0;
	else
		full_raw_cap = 1;
...
result_out:
	...
	wdata = DEVICE_RESET_CMD; /* reset */
	DO_SAFE(sic_i2c_write(ts->client,
		tc_device_ctl, (u8 *)&wdata, sizeof(u32)), error);
	mdelay(80);
	sic_ts_init(ts->client);

	ret = sprintf(buf, "========RESULT=======\n");

	ret += sprintf(buf+ret, "Channel Status : %s", (trx_to_trx == 0 &&  high_resistance == 0) ? "Pass\n" : "Fail\n");

	if (!(trx_to_trx && high_resistance)) {
		ret += sprintf(buf+ret, " (");
		ret += sprintf(buf+ret, "%s /%s", (trx_to_trx == 0 ? " 1" : " 0"),
			(high_resistance == 0 ? " 1" : " 0"));
		ret += sprintf(buf+ret, " )\n");
	}
...
static ssize_t show_lpwg_sd(struct i2c_client *client, char *buf)
{
	...
	/* doze2 = 0x04, LOW_POWER_DISABLE */
	ts->debug_mode = 0x04;
	DO_SAFE(tci_control(ts, DOZE2_MODE_SET), error);
	TOUCH_I("%s DOZE2_MODE_SET Setting\n", __func__);
	...
	len = sprintf(temp_buf, "[T%d]LPWG_Raw_Capacitance_Test",wdata);
	...
	len = sprintf(temp_buf, "LPWG_Raw_Cap_%%\n");

	for ( j = 0; j< tx_cnt; j++) {
		len += sprintf(temp_buf + len, "r_%dt%%                  ", j+1);
		for ( i = 0; i< rx_cnt; i++) {
			lower_adc = sicImage[j][i] * 1000 / sensor_raw_cap_ref[j * tx_cnt + i];
			len += sprintf(temp_buf + len, "%7d.%d",
				lower_adc / 10, lower_adc % 10);
			if ( lower_adc > max_result )
				max_result = lower_adc;
	...
	len = sprintf(temp_buf+len, "LPWG_Raw_Cap_%%               %7d.%d%7d.%d\n",
		max_result / 10, max_result % 10, min_result / 10, min_result % 10);

	write_file (NULL, temp_buf, 0, 1);
	write_file (NULL, "", 1, 1);

	TOUCH_I("%s LPWG_Raw Capacitance Test result (%x)\n", __func__, fail_cnt == 0 ? 1:0);
	if (fail_cnt > 0)
		full_raw_cap = 0;
	else
		full_raw_cap = 1;
...
//	ret += sprintf(buf+ret, "LPWG Raw Data : %s", (full_raw_cap > 0) ? "Pass\n" : "Fail\n");
	ret = sprintf(buf, "LPWG RawData : %s", (full_raw_cap > 0) ? "Pass\n" : "Fail\n");

	if (temp_buf)
		kfree(temp_buf);

	DO_SAFE(tci_control(ts, LOW_POWER_CTRL), error);
	TOUCH_I("%s LOW_POWER_CTRL Setting\n", __func__);
...
static ssize_t show_rawdata_cic(struct i2c_client *client, char *buf)
{
	struct sic_ts_data *ts =
			(struct sic_ts_data *)get_touch_handle(client);
	int ret = 0;
	...
	ret2 = get_data(ts, delta, 1);  /* 1 == rawdata */
	if (ret2 < 0) {
		TOUCH_E("Test fail (Check if LCD is OFF)\n");
		ret += snprintf(buf + ret, PAGE_SIZE - ret,
				"Test fail (Check if LCD is OFF)\n");
						...
						const char *buf, size_t count)
{
	...
	for (p = 0; p < flash_size; p += BURST_SIZE) {
		if ((p%1024) == 0)
			TOUCH_D(DEBUG_BASE_INFO,
				"fw page[%d] Firmware Reading...\n",
				p/1024);
		if ((p+BURST_SIZE) < flash_size)
			j = BURST_SIZE;
		else
			j = flash_size - p;
...
static LGE_TOUCH_ATTR(rawdata_cic, S_IRUGO | S_IWUSR, show_rawdata_cic, NULL);
...
static struct attribute *sic_ts_attribute_list[] = {
	...
	&lge_touch_attr_rawdata_cic.attr,
...
static u16 checksum16(u16 *rdata, int image_size, int cnt)
{
	int i;
	u16 checksum = 0;
	for (i = 0; i < cnt; i++) {
					...
					const struct firmware *fw_entry)
{
	...
	u32 j = 0;
	u16 checksum = 0;
	static u32 cfg_buf[256];
	u32 checksum_2s_complmt = 0;

	/*check if the img has valid info of size*/
	my_image_size = fw_entry->size;
	my_image_bin = (unsigned char *)(fw_entry->data);

	TOUCH_D(DEBUG_BASE_INFO,
			"firmware img read size : 0x%lx\n", my_image_size);
	TOUCH_D(DEBUG_BASE_INFO,
			"firmware img read start :  0x%x\n", fw_entry->data[0]);

	if (my_image_size > max_fw_size) {
		TOUCH_I("FW_IMAGE SIZE is invalid(%ld)", my_image_size);
		goto error;
	}
	...
	DO_SAFE(sic_i2c_write(ts->client,
		fc_addr, (u8 *)&wdata, sizeof(u32)), error);
	wdata = FC_CMD_MASS_ERASE;
	DO_SAFE(sic_i2c_write(ts->client,
		fc_ctl, (u8 *)&wdata, sizeof(u32)), error);

	TOUCH_D(DEBUG_BASE_INFO, "i2c_burst_size : %d\n",
			fw_i2c_burst_write_size);

	if (fw_i2c_burst_write_size > 4) {
		DO_SAFE(sic_i2c_read(ts->client, serial_if_ctl,
				(u8 *)&rdata, sizeof(u32)), error);
		wdata = rdata & ~0x10;
		TOUCH_D(DEBUG_BASE_INFO, "serial if ctrl reg : 0x%x\n", wdata);
	...
	DO_SAFE(sic_i2c_write(ts->client,
			fc_addr, (u8 *)&wdata, sizeof(u32)), error);
	for (p = 0; p < my_image_size; p += fw_i2c_burst_write_size) {
		if ((p%1024) == 0)
			TOUCH_D(DEBUG_BASE_INFO,
				"fw page[%d] Firmware Flashing...\n",
				p/1024);
		if ((p+fw_i2c_burst_write_size) < my_image_size)
			j = fw_i2c_burst_write_size;
		else
			j = my_image_size - p;
		DO_SAFE(sic_i2c_write(ts->client,
	...
	/* checksum calc */
	checksum = checksum16((u16 *)&my_image_bin[0], my_image_size, MAX_NUM_OF_FW_PAGES*512-2);
	checksum_2s_complmt = (~checksum + 1) & 0xFFFF;

	TOUCH_D(DEBUG_BASE_INFO | DEBUG_FW_UPGRADE,
				"fw checksum : 0x%x, 2's complmt 0x%x\n", checksum, checksum_2s_complmt);

	/* fw checksum addr */
	wdata = MAX_NUM_OF_FW_PAGES*256-1;
	DO_SAFE(sic_i2c_write(ts->client,
		fc_addr, (u8 *)&wdata, sizeof(u32)), error);

	/* fw checksum wdata */
	DO_SAFE(sic_i2c_write(ts->client,
		fc_wdata, (u8 *)&checksum_2s_complmt, sizeof(u32)), error);

	/* cfg page addr */
	wdata = MAX_NUM_OF_FW_PAGES*256;
	DO_SAFE(sic_i2c_write(ts->client,
		fc_addr, (u8 *)&wdata, sizeof(u32)), error);
	...
	/* cfg page data read (backup)*/
	for (j = 0; j < 254; j++) {
		DO_SAFE(sic_i2c_read(ts->client,
			fc_rdata, (u8 *)&cfg_buf[j], sizeof(u32)), error);
	}
	checksum = checksum16((u16 *)&cfg_buf[0], 1016, 508);
	checksum_2s_complmt = (~checksum + 1) & 0xFFFF;
	TOUCH_D(DEBUG_BASE_INFO | DEBUG_FW_UPGRADE,
				"cfg checksum : 0x%x, 2's complmt 0x%x\n", checksum, checksum_2s_complmt);

	cfg_buf[254] = checksum_2s_complmt;
	cfg_buf[255] = checksum;
	...
	/* cfg data restore(rewrite) */
	for (j = 0; j < 256; j++) {
		DO_SAFE(sic_i2c_write(ts->client,
			fc_wdata, (u8 *)&cfg_buf[j], sizeof(u32)), error);
	}
	...
	do {
		...
		TOUCH_D(DEBUG_BASE_INFO,
			"checksum busy checking %d.\n", (rdata >> 1) & 1);
		usleep(100000);
	} while (((rdata >> 1) & 1) == 1);
	TOUCH_D(DEBUG_BASE_INFO, "checksum check done!\n");
	...
	if (rdata == 0) {
		TOUCH_D(DEBUG_BASE_INFO | DEBUG_FW_UPGRADE,
				"firmware download okay with checksum result : 0x%x\n",
				rdata);
	} else {
		TOUCH_D(DEBUG_BASE_INFO | DEBUG_FW_UPGRADE,
				"firmware download failed with checksum result : 0x%x\n",
				rdata);
		ret = -EIO;
	}
	return ret;
...
int check_tci_debug_result(struct sic_ts_data *ts, u8 wake_up_type)
{
	...
	if (ts->fail_reason[0]) {
		...
		result = sic_i2c_read(ts->client, 0xD6B0,
                                        (u8 *)&rdata, sizeof(u32));
		if(!result) {
			TOUCH_I("doze2_abnormal_cnt = %d\n", rdata);
		}
		else {
			TOUCH_I("doze2_abnormal_cnt i2c read fail!\n");
		}
		for (i = 0; i < rdata; i++) {
			result = sic_i2c_read(ts->client, 0xD6B1+i,
                                        (u8 *)&addr, sizeof(u32));
			if(!result) {
				TOUCH_I("doze2_abnormal_value[%d] = %d\n", i, addr);
			}
			else {
				TOUCH_I("doze2_abnormal_value i2c read fail!\n");
			}
		}
	}
	return 0;
	...
	struct state_info *state)
{
	...
	if (ts->pdata->pwr->use_regulator == 1) {
		...
		if (ts->pdata->pwr->vio_voltage > 0)
			DO_SAFE(regulator_set_voltage(ts->regulator_vio,
				...
				ts->pdata->pwr->vio_voltage), error);
	} else if (ts->pdata->pwr->use_regulator == 2) { /* GPIO */
		gpio_request(ts->pdata->vdd_pin, "touch_vddh");
		gpio_direction_output(ts->pdata->vdd_pin, 1);
		gpio_request(ts->pdata->vio_pin, "touch_vddl");
	...
	INIT_DELAYED_WORK(&ts->work_second_int, ignore_second_int);

	mutex_init(&ts->mfts_fw_upgrade);
...
enum error_type sic_ts_remove(struct i2c_client *client)
{
	...
	} else if (ts->pdata->pwr->use_regulator == 2) {
		gpio_free(ts->pdata->vio_pin);
		gpio_free(ts->pdata->vdd_pin);
	}

	mutex_destroy(&ts->mfts_fw_upgrade);
	...
	struct touch_data *curr_data, const struct touch_data *prev_data)
{
	...
	if (abt_monitor_head) {
		...
		tHeadBuffer.rn_min = 800;
		tHeadBuffer.rn_max = 1500;
	...
	if (is_first_INT) {
		TOUCH_I("Ignore First Interrupt when Power off\n");
		is_first_INT = 0;
		return IGNORE_EVENT;
	}

	if (is_second_INT) {
		TOUCH_I("Ignore Second Interrupt when Power on\n");
		is_second_INT = 0;
		return IGNORE_EVENT;
	}
	...
	DO_SAFE(sic_i2c_read(client, tc_status2, (u8 *)interrupt_data,
				sizeof(u32)*5), error);
	...
	if (interrupt_status_reg & INTERRUPT_MASK_CUSTOM) {
			...
			if (get_swipe_data(client) == 0) {
				...
				if (ts->pdata->lockscreen_stat == 1) {
					send_uevent_lpwg(client,
						LPWG_SWIPE_DOWN);
					swipe_disable(ts);
					tci_control(ts, LOW_POWER_DISABLE);
				} else {
					ts->pdata->swipe_pwr_ctr =
						WAIT_SWIPE_WAKEUP;
		...
		if (is_normal_operation) {
			...
			/* palm bit */
			TOUCH_D(DEBUG_GET_DATA, "palmbit = %d\n",ts->ts_data.report.touchInfo.palmBit);
			...
			for (i = 0; i < ts->ts_data.report.touchInfo.touchCnt;
				...
				if ((t_Data[i].event == TOUCHSTS_MOVE ||
					...
					/*
					* Change Width Minor Value for Pen Recognition Filter
					* MIN WIDTH MINOR = 15
					*/
					if (c_data->type == TOOL_TYPE_STYLUS)
						c_data->width_minor = STYLUS_WIDTH_MINOR;
					else if (c_data->type != TOOL_TYPE_STYLUS
						&& t_Data[i].width_minor < STYLUS_MAX_WIDTH_MINOR)
						c_data->width_minor = STYLUS_MAX_WIDTH_MINOR;
					else
						c_data->width_minor = t_Data[i].width_minor;
					...
					if (t_Data[i].width_major
						...
						c_data->orientation =
							t_Data[i].angle;
					/*
					* Change Pressure Value for Pen Recognition Filter
					* MAX PEN PRESSURE = 35
					*/
					if (ts->ts_data.report.touchInfo.palmBit & PALM_STATUS
						&& ts->ts_data.report.touchInfo.palmBit >> c_data->id & 0x01)
						c_data->pressure = MAX_PRESSURE;
					else if (c_data->type == TOOL_TYPE_STYLUS
						&& !(ts->ts_data.report.touchInfo.palmBit >> c_data->id & 0x01))
						c_data->pressure = STYLUS_PRESSURE;
					else if (c_data->type != TOOL_TYPE_STYLUS
						&& !(ts->ts_data.report.touchInfo.palmBit >> c_data->id & 0x01)
						&& t_Data[i].byte.pressure < STYLUS_MAX_PRESSURE)
						c_data->pressure = STYLUS_MAX_PRESSURE;
					else
						c_data->pressure = t_Data[i].byte.pressure;
					...
					TOUCH_D(DEBUG_GET_DATA,
						"<%d> type[%d] pos(%4d,%4d) w_m[%2d] w_n[%2d] o[%2d] p[%2d] e[%d]\n",
						t_Data[i].track_id,
						t_Data[i].toolType,
...
//						t_Data[i].byte.pressure,
					...
					f_index++;
						...
						t_Data[i].track_id >= 0 &&
						t_Data[i].track_id < MAX_REPORT_FINGER) {
					touchUpCnt++;
					TOUCH_D(DEBUG_GET_DATA,
					...
					TOUCH_I("Invalid touch data TC[%d] i[%d] e[%d] id[%d]!!\n", ts->ts_data.report.touchInfo.touchCnt, i, t_Data[i].event, t_Data[i].track_id);
				}
			}
...
enum error_type sic_ts_power(struct i2c_client *client, int power_ctrl)
{
	...
	case POWER_OFF:
		...
		if (ts->pdata->pwr->use_regulator == 1) {
			...
			gpio_direction_output(ts->pdata->vdd_pin, 0);
		}
		msleep(ts->pdata->role->reset_delay);
		break;
	case POWER_ON:
					...
					struct touch_firmware_module *fw)
{
	...
	if (!strncmp(ts->fw_info.fw_product_id, "lg23lg23", sizeof(ts->fw_info.fw_product_id))) {
		TOUCH_D(DEBUG_BASE_INFO, "product_id[%s(ic)]: Start initial fw upgrade\n",
			ts->fw_info.fw_product_id);
		id_compare |= 1;
	}
...
enum error_type sic_ts_resume(struct i2c_client *client)
{
	...
	if (mfts_mode) {
		mutex_lock(&ts->mfts_fw_upgrade);

		TOUCH_I("%s : Touch FW(Under v0.03) Upgrade for MFTS mode.\n", __func__);
		touch_disable_irq(ts->client->irq);
...
#if 0
	/* Do not firmware upgrade on the latest firmware */
		memcpy(info_sic->fw_path, ts->pdata->inbuilt_fw_name,
				sizeof(info_sic->fw_path));

		info_sic->force_upgrade = 1;

		fw_sic->need_upgrade = 1;

		TOUCH_I("%s : Path: %s Image: %s force_upgrade: %d need_upgrade: %d\n",
			__func__,info_sic->fw_path, fw_sic->fw_image,
			info_sic->force_upgrade, fw_sic->need_upgrade);
		sic_ts_fw_upgrade(ts->client, info_sic, fw_sic);

		sic_ts_power(ts->client, POWER_OFF);
		sic_ts_power(ts->client, POWER_ON);
		sic_ts_init(ts->client);
#endif
		mutex_unlock(&ts->mfts_fw_upgrade);
	}
