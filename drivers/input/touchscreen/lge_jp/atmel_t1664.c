...
static int temp_mode = NORMAL_TEMP_MODE;
...
#if 1
     if (!regulator_is_enabled(data->vio)) {
...
#if 1
	 if (data->vio) {
		error = regulator_enable(data->vio);
		if (error < 0) {
			TOUCH_INFO_MSG("vio_1V8 regulator enable fail\n");
			return;
		}
	}
#else
...
#if 1
	if (data->vio) {
         error = regulator_disable(data->vio);
         if (error < 0) {
             TOUCH_INFO_MSG("vio_1V8 regulator disable fail\n");
             return;
		}
	}
#else
...
#else
		info_len += snprintf(buf + info_len, PAGE_SIZE - info_len, "Channel Status : %s\n", chstatus_result == 1 ? "PASS" : "PASS");
		info_len += snprintf(buf + info_len, PAGE_SIZE - info_len, "Raw Data : %s\n", rawdata_result == 1 ? "PASS" : "PASS");
#endif
	}
	mxt_power_unblock(POWERLOCK_SYSFS);
...
static ssize_t store_ime_status(struct mxt_data *data, const char *buf, size_t count)
{
	...
	if (value == 1){
		...
		TOUCH_DEBUG_MSG("IME Mode Enable\n");


	} else if (value == 0) {
		mxt_patch_event(global_mxt_data, IME_MODE_DISABLE);
		TOUCH_DEBUG_MSG("IME Mode Disable\n");
		...
		switch(patch_event){
			case HIGH_TEMP_SET: /*13 or 19*/
				...
				if (global_mxt_data->suspended)
					mxt_patch_event(global_mxt_data, HIGH_TEMP_KNOCKON_SET);
				else
					mxt_patch_event(global_mxt_data, HIGH_TEMP_KNOCKON_UNSET);
				break;
