...
struct lge_touch_core_callback_data {
	struct mutex mutex;
...
static struct lge_touch_core_callback_data cb_data;
...
static ssize_t store_mfts_mode(struct i2c_client *client, const char *buf, size_t count)
{
	 int value;
...
static struct touch_platform_data *get_dts_data(struct device *dev)
{
	...
	p_data->vio_pin = of_get_named_gpio_flags(np,
			"vio-gpio", 0, NULL);
	p_data->vdd_pin = of_get_named_gpio_flags(np,
			"vdd-gpio", 0, NULL);
	...
	GET_PROPERTY_U32(np, "use_lpwg_test",
			p_data->role->use_lpwg_test);
		...
		unsigned long event, void *data)
{
	...
	if (evdata && evdata->data && event == FB_EVENT_BLANK) {
		...
		if (*blank == FB_BLANK_UNBLANK) {
			queue_delayed_work(cb_data.workqueue,
					&cb_data.work,
					msecs_to_jiffies(10));
		...
		unsigned long event, void *data)
{
	...
	if (mfts_mode && !ts->pdata->role->mfts_lpwg && (event != LCD_EVENT_TOUCH_PWR_OFF))
		 return 0;

	switch (event) {
	 case LCD_EVENT_TOUCH_PWR_OFF:
		 TOUCH_D(DEBUG_BASE_INFO, "LCD_EVENT_TOUCH_PWR_OFF\n");
		 power_control(ts, POWER_OFF);
		 break;
...
static void lge_touch_core_callback_data_fn_work(struct work_struct *work)
{
	struct lge_touch_data *ts = cb_data.ts;
		...
		const struct i2c_device_id *id)
{
	...
	cb_data.workqueue = create_singlethread_workqueue("touch_core_workqueue");
	INIT_DELAYED_WORK(&cb_data.work, lge_touch_core_callback_data_fn_work);
	cb_data.ts = ts;
