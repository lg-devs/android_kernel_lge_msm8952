...
#include "atmel_2954.h"
#include "atmel_2954_patch.h"
...
static  bool trigger_null_mxt_data = 0;
...
//static bool curr_low_temp = 0; //thermal solution
...
#if 0
static void mxt_temp_monitor_func(struct work_struct *work_temp_monitor)  //thermal solution
{
	struct mxt_data *data = container_of(to_delayed_work(work_temp_monitor), struct mxt_data, work_temp_monitor);
	int temp =0;
	temp= bq24296_get_batt_temp_origin();

	TOUCH_INFO_MSG("mxt_temp_monitor_func temp=%d\n",temp);
	...
	schedule_delayed_work(&data->work_temp_monitor, MONITOR_TEMP_PERIOD);
}
#endif
...
int mxt_get_self_delta_chk(struct mxt_data *data)
{
	...
	for (read_page = 0 ; read_page < 3; read_page++) {
		...
		for (i = 0; i < NODE_PER_PAGE; i++) {
			curr_delta =( ((u16)ret_buf[i*2+1] << 8) + (u16)ret_buf[i*2] ); // À½?? ????
...
//		}
		...
		TOUCH_INFO_MSG("%s : wake_lock_timeout 3000ms for stable interrupt",__func__);
		wake_lock_timeout(&touch_wake_lock, msecs_to_jiffies(3000));
...
void trigger_low_temp_state_from_batt(int low_temp)
{
	...
	TOUCH_INFO_MSG("%s curr_low_temp= %d, low_temp=%d \n", __func__,curr_low_temp,low_temp);
	if(low_temp == 1 && global_mxt_data){
...
/*
	...
	if(lge_get_board_revno() == HW_REV_EVB1){
		TOUCH_INFO_MSG("%s HW rev = evb1.\n",__func__);
		return;
	}

	if (global_mxt_data == NULL){
		TOUCH_INFO_MSG("%s global_mxt_data = NULL.\n",__func__);
		return;
	}

	if(global_mxt_data->charging_mode&&usb_type){
		TOUCH_INFO_MSG("%s already charging mode. return.\n",__func__);
		return;
	}

	if(global_mxt_data->charging_mode==0&&usb_type==0){
		TOUCH_INFO_MSG("already no-charging mode. return.\n");
		return;
	}
*/
	if (global_mxt_data && global_mxt_data->patch.event_cnt) {
		...
		if(usb_type ==1)
			trigger_null_mxt_data = 1;
		else
			trigger_null_mxt_data = 0;
	}
}
...
static int mxt_parse_object_table(struct mxt_data *data)
{
	...
	data->mem_size = 0;
//641t porting S
	TOUCH_INFO_MSG("mxt_parse_object_table\n");
//641t porting E
	for (i = 0; i < data->info->object_num; i++) {
		object = data->object_table + i;
...
static ssize_t mxt_pin_state_show(struct mxt_data *data, char *buf)
{

	return scnprintf(buf, PAGE_SIZE, "RST(%ld):%d INT(%ld):%d, SCL(901):%d, SDA(900):%d LDO1(%ld):%d, LDO2(%ld):%d\n",
			 data->pdata->gpio_reset, gpio_get_value(data->pdata->gpio_reset), data->pdata->gpio_int, gpio_get_value(data->pdata->gpio_int), gpio_get_value(890 + 11), gpio_get_value(890 + 10), data->pdata->gpio_ldo1, gpio_get_value(data->pdata->gpio_ldo1), data->pdata->gpio_ldo2, gpio_get_value(data->pdata->gpio_ldo2));
}
...
static ssize_t mxt_info_show(struct mxt_data *data, char *buf)
{
	...
	if(data->info->family_id==0xA4 && data->info->variant_id==0x0D){
...
static ssize_t mxt_run_self_diagnostic_show(struct mxt_data *data, char *buf)
{
	...
	TOUCH_INFO_MSG("Force cal for self_diagnostic test.\n");
	mxt_t6_command(data, MXT_COMMAND_CALIBRATE, 1, false);
	msleep(30);
...
static ssize_t store_lpwg_notify(struct mxt_data *data, const char *buf, size_t count)
{
	...
	case 8 :
		...
		break;
	case 9 :
		if (value[1] == 0 && resume_flag == 1){
			TOUCH_INFO_MSG("%s : call suspend.\n", __func__);
			mxt_fb_suspend(data);
		}
		else if (value[1] == 1 && resume_flag == 0){
			TOUCH_INFO_MSG("%s : call resume.\n", __func__);
			mxt_fb_resume(data);
		}
		break;
	default:
...
static LGE_TOUCH_ATTR(pin_state, S_IRUGO, mxt_pin_state_show, NULL);
...
static LGE_TOUCH_ATTR(pen_support, S_IRUGO | S_IWUSR, show_pen_support, NULL); //pen
...
static struct attribute *lge_touch_attribute_list[] = {
	...
	&lge_touch_attr_pen_support.attr,//pen
	&lge_touch_attr_ime_status.attr,//ime
...
//	input_set_abs_params(input_dev, ABS_MT_TRACKING_ID, 0, data->pdata->numtouch, 0, 0);
	...
	TOUCH_INFO_MSG("[%s]t100_aux_area=%d, t100_aux_ampl=%d,t100_aux_vect=%d ",__func__,data->t100_aux_area,data->t100_aux_ampl,data->t100_aux_vect);
...
static int  mxt_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (data->ts_pinctrl) {
		...
		if (data->ts_pinset_state_active) {
			...
			TOUCH_INFO_MSG("set ts pinctrl active state \n");
		} else {
	...
	TOUCH_INFO_MSG("request_threaded_irq %s  name:%s\n", __func__, client->name);
...
//	INIT_DELAYED_WORK(&data->work_temp_monitor, mxt_temp_monitor_func);
...
//	mxt_stop(data);
	...
	if(trigger_null_mxt_data == 1) {
		trigger_null_mxt_data = 0;
		trigger_usb_state_from_otg(1);
	}

//	schedule_delayed_work(&data->work_temp_monitor, 0);
...
static int mxt_resume(struct device *device)
{
	...
	if (atomic_read(&pm_state) == PM_SUSPEND_IRQ) {
		...
		desc = irq_to_desc(data->client->irq);
		if (desc == NULL) {
...
static int mxt_fb_resume(struct mxt_data *data)
{
	...
	TOUCH_INFO_MSG("%s \n", __func__);
#if 0
	cancel_delayed_work_sync(&data->work_temp_monitor);
	schedule_delayed_work(&data->work_temp_monitor, 0);  // for thermal monitor
#endif
	if (mxt_power_block_get()) {
		TOUCH_INFO_MSG("Still in use. Do nothing \n");
		return 0;
...
static int __init mxt_init(void)
{
	...
	TOUCH_INFO_MSG("%s() \n", __func__);
	if (lge_get_boot_mode() == LGE_BOOT_MODE_CHARGERLOGO) { // Chargerlogo mode
		TOUCH_INFO_MSG("%s : Ignore function in Chargerlogo\n", __func__);
		return -EMLINK;
	}
