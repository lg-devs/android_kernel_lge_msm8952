...
#if defined(TOUCH_DEVICE_LR388K5)
struct of_device_id  LR388K5_MatchTable[2] = {{ .compatible = "unified_driver_ver3,lr388k5", },};
#endif

#if defined(TOUCH_DEVICE_LR388K6)
struct of_device_id  LR388K6_MatchTable[2] = {{ .compatible = "unified_driver_ver3,lr388k6", },};
#endif
...
static struct of_device_id *pDeviceSpecific_MatchTablelist[MAX_DEVICE_SUPPORT] = {
		...
		#if defined(TOUCH_DEVICE_LR388K5)
		&LR388K5_MatchTable[0],
		#endif
	
	#elif defined(TOUCH_MODEL_PH2)

		#if defined(TOUCH_DEVICE_LR388K6)
		&LR388K6_MatchTable[0],
		#endif
...
void TouchPowerModel(int isOn)
{
	...
	TOUCH_LOG("P1V SHARP TEST GPIO CONTROL isOn %d \n", isOn);
    {
		int ret = 0;
		ret = gpio_request(TOUCH_GPIO_POWER_AVDD, "touch_avdd");
	    if(ret == 0 ){
		    gpio_direction_output(TOUCH_GPIO_POWER_AVDD, 1);
		} else{
	        TOUCH_LOG("P1V SHARP TEST GPIO request fail AVDD  \n");
		}

	    ret = gpio_request(TOUCH_GPIO_POWER_VDDIO, "touch_vddio");
	    if(ret == 0 ){
	        gpio_direction_output(TOUCH_GPIO_POWER_VDDIO, 1);
		} else{
	        TOUCH_LOG("P1V SHARP TEST GPIO request fail VDDIO \n");
		}
	}

	if (isOn) {
		gpio_set_value(TOUCH_GPIO_POWER_VDDIO, 1);
		msleep(10);
		gpio_set_value(TOUCH_GPIO_POWER_AVDD, 1);
	}
	else {
		gpio_set_value(TOUCH_GPIO_POWER_AVDD, 0);
		gpio_set_value(TOUCH_GPIO_POWER_VDDIO, 0);
	}

	TOUCH_LOG("P1V SHARP TEST GPIO CONTROL VDDIO %d \n", gpio_get_value(TOUCH_GPIO_POWER_VDDIO));
	TOUCH_LOG("P1V SHARP TEST GPIO CONTROL AVDD %d \n", gpio_get_value(TOUCH_GPIO_POWER_AVDD));

	#elif defined(TOUCH_MODEL_PH2)

	TOUCH_LOG("PH2 GPIO CONTROL isOn %d \n", isOn);
    {
		int ret = 0;
		ret = gpio_request(TOUCH_GPIO_POWER_AVDD, "touch_avdd");
	    if(ret == 0 ){
	        ...
	        TOUCH_LOG("PH2 GPIO request fail AVDD  \n");
		}
	    ...
	    if(ret == 0 ){
	        ...
	        TOUCH_LOG("PH2 GPIO request fail VDDIO \n");
		}
	}
	...
	TOUCH_LOG("PH2 GPIO CONTROL VDDIO %d \n", gpio_get_value(TOUCH_GPIO_POWER_VDDIO));
	TOUCH_LOG("PH2 GPIO CONTROL AVDD %d \n", gpio_get_value(TOUCH_GPIO_POWER_AVDD));
