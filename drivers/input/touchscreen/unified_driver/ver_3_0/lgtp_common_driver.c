...
#if defined(CONFIG_TOUCHSCREEN_MSM8952_P1V)  || defined(CONFIG_TOUCHSCREEN_MSM8952_PH2)//HMOON TEMP
...
//			pReadData->count, newFinger, changedFinger, pressedFinger, releasedFinger);
	...
	for (i = 0; i < MAX_FINGER; i++) {
		...
		if ((releasedFinger >> i) & 0x1) {
			#if defined(CONFIG_TOUCHSCREEN_MSM8952_P1V)  || defined(CONFIG_TOUCHSCREEN_MSM8952_PH2)
...
#if defined(CONFIG_TOUCHSCREEN_MSM8952_PH2)
...
static int touch_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (pDriverData->ts_pinctrl) {
		pDriverData->ts_pinctrl_state_active =
			pinctrl_lookup_state(pDriverData->ts_pinctrl, "pmx_ts_active");
		...
		pDriverData->ts_pinctrl_state_suspend =
			pinctrl_lookup_state(pDriverData->ts_pinctrl, "pmx_ts_suspend");
