		...
		unsigned short addr, unsigned char *data, unsigned short length)
{
	...
	if (retval < 0) {
		LGTC_ERR("Failed to read from Page_Select register\n");
		return retval;
	}
	...
	if (page_old == page_new) {
		...
		if (retval < 0) {
			LGTC_ERR("Failed to change Page_Select register\n");
			return retval;
		}
	...
	if (retval < 0) {
		LGTC_ERR("Failed to read the register(addr=0x%04x)\n", addr);
		return retval;
	}
	...
	if (page_changed) {
		...
		if (retval < 0) {
			LGTC_ERR("Failed to restore Page_Select register\n");
			return retval;
		}
	}
		...
		unsigned short addr, unsigned char *data, unsigned short length)
{
	...
	if (retval < 0) {
		LGTC_ERR("Failed to write to the register(addr=0x%04x)\n", addr);
		return retval;
	}
...
/* page read */
	...
	if (retval < 0) {
		LGTC_ERR("Failed to long read the register(addr=0x%04x)\n", addr);
		return retval;
	}
		...
		char *buf, loff_t pos, size_t count)
{
	...
	if (length > (REG_ADDR_LIMIT - address)) {
		LGTC_ERR("[%s] Out of register map limit\n", __FUNCTION__);
		return -EINVAL;
	}
	...
	if (length) {
		retval = synaptics_rmi4_reg_read(rmidev->ts_data,
				...
				length);
		if (retval < 0) {
			LGTC_ERR("[%s]  Failed to read data\n", __FUNCTION__);
			return retval;
		}
	} else {
		return -EINVAL;
		...
		char *buf, loff_t pos, size_t count)
{
	...
	if (length) {
		retval = synaptics_rmi4_reg_write(rmidev->ts_data,
				...
				length);
		if (retval < 0) {
			LGTC_ERR("[%s] Failed to write data\n", __FUNCTION__);
			return retval;
		}
	} else {
		return -EINVAL;
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (rmidev->pid) {
		...
		if (!rmidev->task) {
			LGTC_ERR("[%s] Failed to locate PID of data logging tool\n", __FUNCTION__);
			return -EINVAL;
		}
	}
...
static loff_t rmidev_llseek(struct file *filp, loff_t off, int whence)
{
	...
	if (IS_ERR(dev_data)) {
		LGTC_ERR("[%s] Pointer of char device data is invalid\n",
					__FUNCTION__);
		return -EBADF;
	}
	...
	if (newpos < 0 || newpos > REG_ADDR_LIMIT) {
		LGTC_ERR("[%s] New position 0x%04x is invalid\n",
					__FUNCTION__, whence);
		newpos = -EINVAL;
		goto clean_up;
	}
...
static void rmidev_device_cleanup(struct rmidev_data *dev_data)
{
	...
	if (dev_data) {
		...
		LGTC_DBG("[%s] rmidev device removed\n", __FUNCTION__);
	}
...
static int rmidev_init_device(struct synaptics_ts_data *ts)
{
	...
	if (!rmidev) {
		LGTC_ERR("[%s] Failed to alloc mem for rmidev\n", __FUNCTION__);
		retval = -ENOMEM;
		goto err_rmidev;
	}
	...
	if (retval < 0) {
		LGTC_ERR("[%s] ailed to create device class\n", __FUNCTION__);
	...
	if (rmidev_major_num) {
		...
		if (retval < 0) {
			LGTC_ERR("[%s] Failed to allocate char device region\n", __FUNCTION__);
			goto err_device_region;
		}

		rmidev_major_num = MAJOR(dev_no);
		LGTC_DBG("[%s] Major number of rmidev = %d\n",
					__FUNCTION__, rmidev_major_num);
	}

	dev_data = kzalloc(sizeof(*dev_data), GFP_KERNEL);
	if (!dev_data) {
		LGTC_ERR("[%s] Failed to alloc mem for dev_data\n", __FUNCTION__);
		retval = -ENOMEM;
		goto err_dev_data;
	}
	...
	if (retval < 0) {
		LGTC_ERR("[%s] Failed to add rmi char device\n", __FUNCTION__);
	...
	if (IS_ERR(device_ptr)) {
		LGTC_ERR("[%s] Failed to create rmi char device\n", __FUNCTION__);
		retval = -ENODEV;
		goto err_char_device;
	}
	...
	if (retval < 0) {
		LGTC_ERR("[%s] Failed to export attention gpio\n", __FUNCTION__);
	} else {
		retval = gpio_export_link(&(lge_ts->input_dev->dev), "attn", ts->pdata->int_pin);

		if (retval < 0) {
			LGTC_ERR("[%s] Failed to create gpio symlink\n", __FUNCTION__);
		} else {
			LGTC_DBG("[%s] Exported attention gpio %d\n",
	...
	if (!rmidev->sysfs_dir) {
		LGTC_ERR("[%s] Failed to create sysfs directory\n", __FUNCTION__);
		retval = -ENODEV;
		goto err_sysfs_dir;
	}
	...
	retval = sysfs_create_bin_file(rmidev->sysfs_dir,
			&attr_data);
	if (retval < 0) {
		LGTC_ERR("[%s] Failed to create sysfs bin file\n", __FUNCTION__);
		goto err_sysfs_bin;
	}
	...
	for (attr_count = 0; attr_count < ARRAY_SIZE(attrs); attr_count++) {
		...
		if (retval < 0) {
			LGTC_ERR("[%s] Failed to create sysfs attributes\n", __FUNCTION__);
			retval = -ENODEV;
