...
static struct lsm303c_mag_platform_data default_lsm303c_mag_pdata = {
	.poll_interval = 100,
	.min_interval = LSM303C_MAG_MIN_POLL_PERIOD_MS,
	.fs_range = LSM303C_MAG_FS_16G,
	.axis_map_x = 0,
	.axis_map_y = 1,
	.axis_map_z = 2,
	.negate_x = 0,
...
static int lsm303c_hw_init(struct lsm303c_status *stat)
{
	...
	err = lsm303c_i2c_write(stat, buf, 5);
	if (err < 0) {
		dev_warn(&stat->client->dev,
			       ...
			       struct device_attribute *attr, char *buf)
{
	...
	lsm303c_mag_device_power_on(stat);

	x[0] = REG_CNTRL1_ADDR;
	x[1] = 0x1c;
	x[2] = 0x60;
	lsm303c_i2c_write(stat, x, 2);

	mdelay(20);

	x[0] = REG_CNTRL3_ADDR;
	x[1] = 0x00;
	lsm303c_i2c_write(stat, x, 1);

	mdelay(20);

	x[0] = REG_MAG_DATA_ADDR;
	lsm303c_i2c_read(stat, x, 6);

	for (i = 0; i < 6; i++) {
		while (1) {
			x[0] = 0x27;
			val = lsm303c_i2c_read(stat, x, 1);
			if (val < 0) {
...
ST_EXIT:
	...
	if (en_state) lsm303c_mag_enable(stat);
...
static struct device_attribute attributes[] = {
	...
	__ATTR(poll_delay, 0666, attr_get_polling_rate_mag, attr_set_polling_rate_mag),
...
int lsm303c_mag_input_open(struct input_dev *input)
{
	...
	return 0;//lsm303c_mag_enable(stat);
}
...
static int lsm303c_mag_get_data(struct lsm303c_status *stat, int *xyz)
{
	...
	//printk(KERN_WARNING "k303c_mag2 = %d,%d,%d\n", hw_d[0], hw_d[1], hw_d[2]);
...
static int lsm303c_mag_input_init(struct lsm303c_status *stat)
{
	...
	stat->input_dev_mag->name = LSM303C_MAG_DEVICE_CUSTOM_NAME;
		...
		const struct i2c_device_id *id)
{
	...
	if (lge_hw_revision == HW_REV_A || lge_hw_revision == HW_REV_B) {
		/*HW_REV_A == 3. Rev 0*/
		/*HW_REV_B == 4. Rev A*/
		default_lsm303c_mag_pdata.negate_x = 1;
		default_lsm303c_mag_pdata.negate_y = 1;
		default_lsm303c_mag_pdata.negate_z = 1;
	} else if (lge_hw_revision == HW_REV_HDKA) {
		/*HDK Power Board.*/
		default_lsm303c_mag_pdata.negate_x = 0;
		default_lsm303c_mag_pdata.negate_y = 1;
		default_lsm303c_mag_pdata.negate_z = 0;
	} else {
		default_lsm303c_mag_pdata.negate_x = 1;
		default_lsm303c_mag_pdata.negate_y = 1;
		default_lsm303c_mag_pdata.negate_z = 1;
...
#ifdef CONFIG_PM
static int lsm303c_mag_resume(struct i2c_client *client)
{
	struct lsm303c_status *stat = i2c_get_clientdata(client);
...
static int lsm303c_mag_suspend(struct i2c_client *client, pm_message_t mesg)
{
	struct lsm303c_status *stat = i2c_get_clientdata(client);

	stat->on_before_suspend = atomic_read(&stat->enabled_mag);
	return lsm303c_mag_disable(stat);
}
#else
#define lsm303c_mag_suspend	NULL
#define lsm303c_mag_resume	NULL
#endif /* CONFIG_PM */
...
static struct i2c_driver lsm303c_driver = {
	...
	.suspend = lsm303c_mag_suspend,
	.resume = lsm303c_mag_resume,
	.id_table = lsm303c_id,
};
