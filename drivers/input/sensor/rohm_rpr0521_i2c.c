...
/* ssoon.lee@lge.com */
#define PS_CALIBRATION_PATH "/persist/sns/prox_calibration.dat"
...
static ssize_t rpr0521_store_run_calibration(struct device *dev, struct device_attribute *attr,
												const char *buf, size_t count)
{
	...
	cal_file_status = 0; // need to new value applied. reset CAL_APPLIED.
...
static ssize_t rpr0521_store_enable(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (val == 1) {
		if (is_proximity) {
			if (cal_file_status != CAL_APPLIED) {
				ps_als_ginfo->cross_talk = read_calibration_data_from_fs();
				cal_file_status = CAL_APPLIED;
			}
			/* initialize current state to far for the setting of initial threshold */
			update_ps_value(PS_THRESHOLD_MIN);
			input_report_abs(ps_als_ginfo->input_dev, ABS_DISTANCE, (ps_als_ginfo->ps_value == true) ? 0 : 5);
			input_sync(ps_als_ginfo->input_dev);
			ps_als_ginfo->enable = PS_ALS_ENABLE;
		} else if (is_light) {
			input_report_abs(ps_als_ginfo->input_dev_als, ABS_MISC, 43001);
			input_sync(ps_als_ginfo->input_dev);
			/* start timer */
			result = hrtimer_start(&ps_als_ginfo->timer, ktime_set(0, 0), HRTIMER_MODE_REL);
...
static ssize_t rpr0521_store_near_offset(struct device *dev, struct device_attribute *attr,
											const char *buf, size_t count)
{
	unsigned long val = simple_strtoul(buf, NULL, 10);
...
static ssize_t rpr0521_store_far_offset(struct device *dev, struct device_attribute *attr,
											const char *buf, size_t count)
{
	unsigned long val = simple_strtoul(buf, NULL, 10);
...
static ssize_t rpr0521_store_led_current(struct device *dev, struct device_attribute *attr,
											const char *buf, size_t count)
{
	unsigned long val = simple_strtoul(buf, NULL, 10);
...
static ssize_t rpr0521_store_infrared_level(struct device *dev, struct device_attribute *attr,
												const char *buf, size_t count)
{
	unsigned long val = simple_strtoul(buf, NULL, 10);
...
static ssize_t rpr0521_show_psth_upper(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", ps_als_ginfo->adjusted_psth_upper);
}

static ssize_t rpr0521_store_psth_upper(struct device *dev, struct device_attribute *attr,
		const char *buf, size_t count)
{
	unsigned long val;
	int err;
	...
	ps_als_ginfo->adjusted_psth_upper = val;
	return count;
}

static ssize_t rpr0521_show_psth_low(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", ps_als_ginfo->adjusted_psth_low);
}

static ssize_t rpr0521_store_psth_low(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long val;
	int err;

	err = kstrtoul(buf, 10, &val);
	ps_als_ginfo->adjusted_psth_low = val;
	return count;
}

static ssize_t rpr0521_show_als_poll_delay(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", ps_als_ginfo->delay_time);
}

static ssize_t rpr0521_store_als_poll_delay(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long val;
	int err;
...
static ssize_t rpr0521_show_als_gain(struct device *dev, struct device_attribute *attr, char *buf)
{
...
static ssize_t rpr0521_show_lux_value(struct device *dev, struct device_attribute *attr, char *buf)
{
...
static DEVICE_ATTR(psth_upper, S_IRUGO | S_IWUSR | S_IWGRP, rpr0521_show_psth_upper, rpr0521_store_psth_upper);
static DEVICE_ATTR(psth_low, S_IRUGO | S_IWUSR | S_IWGRP, rpr0521_show_psth_low, rpr0521_store_psth_low);
static DEVICE_ATTR(als_poll_delay, S_IRUGO | S_IWUSR | S_IWGRP, rpr0521_show_als_poll_delay, rpr0521_store_als_poll_delay);
static DEVICE_ATTR(als_gain, S_IRUGO | S_IWUSR | S_IWGRP, rpr0521_show_als_gain, NULL);
static DEVICE_ATTR(lux_value, S_IRUGO | S_IWUSR | S_IWGRP, rpr0521_show_lux_value, NULL);
...
static struct attribute *ps_attributes[] = {
	...
	&dev_attr_psth_upper.attr,
	&dev_attr_psth_low.attr,
	NULL
};
 ...
 ************************************************************/
/******************************************************************************
 * NAME		: ps_als_parse_dt
 * FUNCTION	: parse device tree
 * REMARKS	: ssoon.lee@lge.com
 *****************************************************************************/
#ifdef CONFIG_OF
static int ps_als_parse_dt(struct device *dev, PS_ALS_DATA *ps_als)
{
 ...
 ************************************************************/
/******************************************************************************
 * NAME		: ps_als_set_regulator
 * FUNCTION	: set regulator
 * REMARKS	: minkyeong07.kim@lge.com
 *****************************************************************************/
static int ps_als_set_regulator(PS_ALS_DATA *ps_als)
{
	int rc;
	...
	np = ps_als->client->dev.of_node;
	if (!np)
		return -EINVAL;

	ps_als->vddio_reg = regulator_get(&ps_als->client->dev, "Rohm,vcc_i2c");
	if (IS_ERR(ps_als->vddio_reg)) {
		rc = PTR_ERR(ps_als->vddio_reg);
		dev_err(&ps_als->client->dev,
			"Regulator get failed vcc_i2c rc=%d\n", rc);
		return rc;
	}

	if (regulator_count_voltages(ps_als->vddio_reg) > 0) {
		rc = regulator_set_voltage(ps_als->vddio_reg,
						1800000,
						1800000);
	}

	ps_als->vdd_reg = regulator_get(&ps_als->client->dev, "Rohm,vdd");
	if (IS_ERR(ps_als->vdd_reg)) {
		rc = PTR_ERR(ps_als->vdd_reg);
		dev_err(&ps_als->client->dev,
			"Regulator get failed vdd rc=%d\n", rc);
		return rc;
	}
	if (regulator_count_voltages(ps_als->vdd_reg) > 0) {
		rc = regulator_set_voltage(ps_als->vdd_reg,
						3000000,
						3000000);
	}

	rc = regulator_enable(ps_als->vddio_reg);
	rc = regulator_enable(ps_als->vdd_reg);
...
static int read_calibration_data_from_fs(void)
{
	...
	if (fd >= 0) {
		...
		PINFO("Read calibration value from fs successfully. cross-talk is (%d)",
					(int)simple_strtoul(read_buf, NULL, 10));
		if (len <= 0) {
			ret = -1;
...
#if 1 // hysteresis = config.init_data.psth_upper - config.init_data.psth_low
	ps_als_ginfo->adjusted_psth_upper = ps_als_ginfo->config.init_data.psth_upper + ret;
	ps_als_ginfo->adjusted_psth_low = ps_als_ginfo->config.init_data.psth_low + ret;
	PINFO("adjusted_psth_upper = %d, adjusted_psth_low = %d",
				ps_als_ginfo->adjusted_psth_upper, ps_als_ginfo->adjusted_psth_low);
#else
	ps_als_ginfo->adjusted_psth_upper = ps_als_ginfo->config.init_data.psth_upper + ret;
...
static int lux_calculation(void)
{
	...
	PDEBUG("rpr0521_lux_calculation : keep_lux = %ld", keep_lux);
	PDEBUG("rpr0521_lux_calculation : lux_result = %d", lux);
...
static int update_ps_value(unsigned short new_ps_data)
{
	...
	if (new_ps_data > ps_als_ginfo->adjusted_psth_upper) {	/* current state is near */
		...
		PDEBUG("pdata(%d) > adjusted_near_offset(%d), proximity state is changed far to near",
		...
		PDEBUG("pdata(%d) < adjusted_far_offset(%d), proximity state is changed near to far",
		...
		PDEBUG("proximity state is not changed. pdata(%d)", new_ps_data);
	}
...
static void ps_als_work_func(struct work_struct *work)
{
	...
	if (pwr_st.ps_state == CTL_STANDALONE && ps_als_ginfo->is_interrupt) {
		...
		input_report_abs(ps_als->input_dev, ABS_DISTANCE, (ps_als_ginfo->ps_value == true) ? 0 : 5);
...
#define ROHM_PSALS_PSMAX	(1)		/* 0 = near, 1 = far */
	...
	if (client->dev.of_node) {
		err = ps_als_parse_dt(&client->dev, ps_als);
		if (err)
			return err;
	}
#endif

	err = ps_als_set_regulator(ps_als);
...
/*********** end light sensor input dev registration ***************/
	...
	result = request_threaded_irq(client->irq, NULL, ps_als_irq_handler, IRQF_TRIGGER_FALLING, client->name, ps_als);
		...
		if (result == 0) {
			...
			PINFO("ps_als_irq_handler = 0x%p\n", ps_als_irq_handler);
			PINFO("interrupt flag	= 0x%x\n", IRQF_TRIGGER_FALLING);
			PINFO("interrupt name	= %s\n", client->name);
			PINFO("base address		= 0x%p\n", ps_als);
		}
...
err_inputdev_als:
	sysfs_remove_group(&ps_als->input_dev_als->dev.kobj, &als_attr_group);
	input_unregister_device(ps_als->input_dev_als);
err_power_failed_als:
	sysfs_remove_group(&ps_als->input_dev->dev.kobj, &ps_attr_group);
