...
static int print_wakeup_source_active_klog(struct wakeup_source *ws)
{
	unsigned long flags;
	ktime_t total_time;
	ktime_t max_time;
	...
	if (ktime_to_ms(active_time) > 0)
		ret = printk(KERN_INFO "%-12s\t%lu\t\t%lu\t\t%lu\t\t%lu\t\t%lu\t\t"
			"%lld\t\t%lld\t\t%lld\t\t%lld\t\t%lld\n",
			ws->name, active_count, ws->event_count,
			ws->wakeup_count, ws->expire_count, ws->pending_count,
			ktime_to_ms(active_time), ktime_to_ms(total_time),
...
int wakeup_sources_active_stats_klog(void)
{
	struct wakeup_source *ws;

	printk(KERN_INFO "name\t\tactive_count\tevent_count\twakeup_count\t"
		"expire_count\tpending_count\tactive_since\ttotal_time\t"
		"max_time\tlast_change\tprevent_suspend_time\n");

	rcu_read_lock();
	list_for_each_entry_rcu(ws, &wakeup_sources, entry)
		print_wakeup_source_active_klog(ws);
	rcu_read_unlock();

	return 0;
}
EXPORT_SYMBOL(wakeup_sources_active_stats_klog);
#endif
...
#ifdef CONFIG_LGE_PM
static int
wakeup_sources_active_stats_open(struct inode *inode, struct file *file)
{
